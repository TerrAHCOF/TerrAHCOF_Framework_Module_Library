...
resource "azurerm_policy_set_definition" "policy_set_definitions" {
  ...
  dynamic "policy_definition_reference" {
    for_each = [
      ...
      {
        policy_definition_id = "${local.provider_path.policy_definitions}${item.policy_definition_id}"
    ...
    description         = try(each.value.description, "${each.value.display_bame} Policy Set Definition at scope ${each.value.management_group_id}")
    management_group_id = try("${local.provider_path.management_groups}${each.value.management_group_id}", null)
